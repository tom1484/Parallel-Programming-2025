...
ScaleSpacePyramid generate_gaussian_pyramid(const Image& img, float sigma_min, int num_octaves, int scales_per_octave) {
    ...
    for (int i = 0; i < num_octaves; i++) {
        ...
        pyramid.octaves[i].push_back(move(base_img));
...
tuple<float, float, float> fit_quadratic(Keypoint& kp, const vector<Image>& octave, int scale) {
    ...
    h12 = (next.get_pixel(x + 1, y, 0) - next.get_pixel(x - 1, y, 0) - prev.get_pixel(x + 1, y, 0) +
           prev.get_pixel(x - 1, y, 0)) *
          0.25;
    h13 = (next.get_pixel(x, y + 1, 0) - next.get_pixel(x, y - 1, 0) - prev.get_pixel(x, y + 1, 0) +
           prev.get_pixel(x, y - 1, 0)) *
          0.25;
    h23 = (img.get_pixel(x + 1, y + 1, 0) - img.get_pixel(x + 1, y - 1, 0) - img.get_pixel(x - 1, y + 1, 0) +
...
bool point_is_on_edge(const Keypoint& kp, const vector<Image>& octave, float edge_thresh = C_EDGE) {
    ...
    h12 = (img.get_pixel(x + 1, y + 1, 0) - img.get_pixel(x + 1, y - 1, 0) - img.get_pixel(x - 1, y + 1, 0) +
           img.get_pixel(x - 1, y - 1, 0)) *
          0.25;
...
vector<Keypoint> find_keypoints(const ScaleSpacePyramid& dog_pyramid, float contrast_thresh, float edge_thresh) {
    ...
    for (int i = 0; i < dog_pyramid.num_octaves; i++) {
        const vector<Image>& octave = dog_pyramid.octaves[i];
...
ScaleSpacePyramid generate_gradient_pyramid(const ScaleSpacePyramid& pyramid) {
    ...
    ScaleSpacePyramid grad_pyramid = {pyramid.num_octaves, pyramid.imgs_per_octave,
                                      vector<vector<Image>>(pyramid.num_octaves)};
        ...
        for (int j = 0; j < pyramid.imgs_per_octave; j++) {
            ...
            for (int x = 1; x < grad.width - 1; x++) {
                for (int y = 1; y < grad.height - 1; y++) {
                    gx = (pyramid.octaves[i][j].get_pixel(x + 1, y, 0) - pyramid.octaves[i][j].get_pixel(x - 1, y, 0)) *
                         0.5;
                    grad.set_pixel(x, y, 0, gx);
                    gy = (pyramid.octaves[i][j].get_pixel(x, y + 1, 0) - pyramid.octaves[i][j].get_pixel(x, y - 1, 0)) *
                         0.5;
...
vector<float> find_keypoint_orientations(Keypoint& kp, const ScaleSpacePyramid& grad_pyramid, float lambda_ori,
                                         float lambda_desc) {
    PROFILE_FUNCTION();
    ...
    float min_dist_from_border = min({kp.x, kp.y, pix_dist * img_grad.width - kp.x, pix_dist * img_grad.height - kp.y});
    ...
    for (int x = x_start; x <= x_end; x++) {
        for (int y = y_start; y <= y_end; y++) {
            ...
            weight =
                exp(-(pow(x * pix_dist - kp.x, 2) + pow(y * pix_dist - kp.y, 2)) / (2 * patch_sigma * patch_sigma));
            theta = fmod(atan2(gy, gx) + 2 * M_PI, 2 * M_PI);
            bin = (int)round(N_BINS / (2 * M_PI) * theta) % N_BINS;
...
void update_histograms(float hist[N_HIST][N_HIST][N_ORI], float x, float y, float contrib, float theta_mn,
    ...
    for (int i = 1; i <= N_HIST; i++) {
        ...
        if (abs(x_i - x) > 2 * lambda_desc / N_HIST) continue;
        for (int j = 1; j <= N_HIST; j++) {
            y_j = (j - (1 + (float)N_HIST) / 2) * 2 * lambda_desc / N_HIST;
            if (abs(y_j - y) > 2 * lambda_desc / N_HIST) continue;

            float hist_weight =
                (1 - N_HIST * 0.5 / lambda_desc * abs(x_i - x)) * (1 - N_HIST * 0.5 / lambda_desc * abs(y_j - y));

            for (int k = 1; k <= N_ORI; k++) {
                float theta_k = 2 * M_PI * (k - 1) / N_ORI;
                float theta_diff = fmod(theta_k - theta_mn + 2 * M_PI, 2 * M_PI);
                if (abs(theta_diff) >= 2 * M_PI / N_ORI) continue;
...
void hists_to_vec(float histograms[N_HIST][N_HIST][N_ORI], array<uint8_t, 128>& feature_vec) {
...
void compute_keypoint_descriptor(Keypoint& kp, float theta, const ScaleSpacePyramid& grad_pyramid, float lambda_desc) {
    ...
    float half_size = sqrt(2) * lambda_desc * kp.sigma * (N_HIST + 1.) / N_HIST;
    ...
    for (int m = x_start; m <= x_end; m++) {
        for (int n = y_start; n <= y_end; n++) {
            ...
            if (max(abs(x), abs(y)) > lambda_desc * (N_HIST + 1.) / N_HIST) continue;
            ...
            float weight =
                exp(-(pow(m * pix_dist - kp.x, 2) + pow(n * pix_dist - kp.y, 2)) / (2 * patch_sigma * patch_sigma));
...
vector<Keypoint> find_keypoints_and_descriptors(const Image& img, float sigma_min, int num_octaves,
                                                int scales_per_octave, float contrast_thresh, float edge_thresh,
...
float euclidean_dist(array<uint8_t, 128>& a, array<uint8_t, 128>& b) {
    float dist = 0;
    for (int i = 0; i < 128; i++) {
        int di = (int)a[i] - b[i];
        dist += di * di;
