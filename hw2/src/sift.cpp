...
bool refine_or_discard_keypoint(Keypoint& kp, const vector<Image>& octave, float contrast_thresh,
                                float edge_thresh) {
    int k = 0;
...
vector<float> find_keypoint_orientations(Keypoint& kp, const ScaleSpacePyramid& grad_pyramid, float lambda_ori,
    ...
    float min_dist_from_border =
        min({kp.x, kp.y, pix_dist * img_grad.width - kp.x, pix_dist * img_grad.height - kp.y});
